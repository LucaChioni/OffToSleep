...
def muri_porte(x, y, nx, ny, stanza, carim, mostro, robo, porte, cofanetti):
    ...
    if (stanza == 1) and ((nx != 0) or (ny != 0)) and not cambiosta:
        # porte
        if ny == -GlobalVar.gpy and x == GlobalVar.gsx // 32 * 6 and y == GlobalVar.gsy // 18 * 2 and not mostro and not robo:
            stanza = 2
            cambiosta = True
            carim = True
        elif ny == -GlobalVar.gpy and x == GlobalVar.gsx // 32 * 6 and y == GlobalVar.gsy // 18 * 1 and not mostro and not robo:
            ny = 0
        elif nx == GlobalVar.gpx and x == GlobalVar.gsx // 32 * 6 and y == GlobalVar.gsy // 18 * 1 and not mostro and not robo:
            nx = 0
        elif nx == -GlobalVar.gpx and x == GlobalVar.gsx // 32 * 6 and y == GlobalVar.gsy // 18 * 1 and not mostro and not robo:
            nx = 0
        else:
        ...
        if oggetto(GlobalVar.gsx // 32 * 7, GlobalVar.gsy // 18 * 11, GlobalVar.gpx * 1, GlobalVar.gpy * 1, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 18, GlobalVar.gsy // 18 * 7, GlobalVar.gpx * 2, GlobalVar.gpy * 2, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 21, GlobalVar.gsy // 18 * 11, GlobalVar.gpx * 4, GlobalVar.gpy * 4, x, y, nx, ny):
            nx = 0
            ny = 0
    ...
    if (stanza == 2) and ((nx != 0) or (ny != 0)) and not cambiosta:
        ...
        # controllo muri-oggetti
        #          posizione x    posizione y    dim x    dim y    px py nx  ny
        if oggetto(GlobalVar.gsx // 32 * 9, GlobalVar.gsy // 18 * 2, GlobalVar.gpx * 1, GlobalVar.gpy * 6, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 2, GlobalVar.gsy // 18 * 7, GlobalVar.gpx * 1, GlobalVar.gpy * 1, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 4, GlobalVar.gsy // 18 * 7, GlobalVar.gpx * 12, GlobalVar.gpy * 1, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 2, GlobalVar.gsy // 18 * 11, GlobalVar.gpx * 6, GlobalVar.gpy * 1, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 7, GlobalVar.gsy // 18 * 13, GlobalVar.gpx * 5, GlobalVar.gpy * 1, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 9, GlobalVar.gsy // 18 * 13, GlobalVar.gpx * 1, GlobalVar.gpy * 3, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 11, GlobalVar.gsy // 18 * 11, GlobalVar.gpx * 1, GlobalVar.gpy * 3, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 13, GlobalVar.gsy // 18 * 11, GlobalVar.gpx * 3, GlobalVar.gpy * 1, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 15, GlobalVar.gsy // 18 * 10, GlobalVar.gpx * 1, GlobalVar.gpy * 2, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 15, GlobalVar.gsy // 18 * 4, GlobalVar.gpx * 1, GlobalVar.gpy * 5, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 15, GlobalVar.gsy // 18 * 2, GlobalVar.gpx * 1, GlobalVar.gpy * 1, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 15, GlobalVar.gsy // 18 * 6, GlobalVar.gpx * 9, GlobalVar.gpy * 1, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 23, GlobalVar.gsy // 18 * 4, GlobalVar.gpx * 4, GlobalVar.gpy * 1, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 23, GlobalVar.gsy // 18 * 6, GlobalVar.gpx * 1, GlobalVar.gpy * 6, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 23, GlobalVar.gsy // 18 * 13, GlobalVar.gpx * 1, GlobalVar.gpy * 3, x, y, nx, ny):
            nx = 0
            ny = 0
        if oggetto(GlobalVar.gsx // 32 * 26, GlobalVar.gsy // 18 * 4, GlobalVar.gpx * 1, GlobalVar.gpy * 3, x, y, nx, ny):
            nx = 0
            ny = 0
...
def trovacasattaccabili(x, y, stanza, porte, cofanetti, raggio):
    ...
    while n < rangeXDestra:
        ...
        while m <= rangeYBasso:
            ...
            nmurx, nmury, stanza, carim, cambiosta = muri_porte(murx, mury, GlobalVar.gpx, 0, stanza, False, True, False, porte, cofanetti)
            if murx != nmurx:
    ...
    while n < rangeYBasso:
        ...
        while m <= rangeXDestra:
            ...
            nmurx, nmury, stanza, carim, cambiosta = muri_porte(murx, mury, 0, GlobalVar.gpy, stanza, False, True, False, porte, cofanetti)
            if mury == nmury:
                i = 0
...
def aperturacofanetto(stanza, cx, cy, dati):
    ...
    if stanza == 1:
        ...
        if cx == GlobalVar.gpx * 7 and cy == GlobalVar.gpy * 12:
            ...
            if tesoro == -1:
                tesoro = -2
        if cx == GlobalVar.gpx * 12 and cy == GlobalVar.gpy * 11:
            c = 0
            i = 101
            while i <= 120:
                if dati[i] == -1:
            ...
            if tesoro == -1:
                tesoro = -2
    if stanza == 2:
        if cx == GlobalVar.gpx * 3 and cy == GlobalVar.gpy * 5:
            c = 0
            i = 101
            while i <= 120:
                if dati[i] == -1:
            ...
            if tesoro == -1:
                tesoro = -2
        if cx == GlobalVar.gpx * 5 and cy == GlobalVar.gpy * 10:
            tesoro = 11
        if cx == GlobalVar.gpx * 10 and cy == GlobalVar.gpy * 9:
            tesoro = 81
...
def scopriCaselleViste(x, y, rx, ry, numstanza, porte, cofanetti, caseviste, escludiPorte=True):
    ...
    while j < len(caselleEsplorate):
        nx, ny, stanza, carim, cambiosta = muri_porte(caselleEsplorate[j], caselleEsplorate[j + 1], 0, -GlobalVar.gpy, numstanza, False, escludiPorte, False, porte, cofanetti)
        ...
        if caselleEsplorate[j] != nx or caselleEsplorate[j + 1] != ny:
            ...
            while i < len(caseviste):
                if caseviste[i] == nx and caseviste[i + 1] == ny:
                    ...
                    break
                i += 3
        nx, ny, stanza, carim, cambiosta = muri_porte(caselleEsplorate[j], caselleEsplorate[j + 1], -GlobalVar.gpx, 0, numstanza, False, escludiPorte, False, porte, cofanetti)
        if caselleEsplorate[j] != nx or caselleEsplorate[j + 1] != ny:
            giaVisitata = False
            k = 0
            while k < len(caselleEsplorate):
...
def disegnaRallo(avanzamentoStoria, npers, x, y, avvele, pers, arma, armatura, scudo, collana, arco, faretra, guanti, inMovimento=False, frame=False, attaccoRavvicinato=False, attaccoDaLontano=False):
    ...
    if attaccoDaLontano:
        if npers == 1:
            ...
            if avanzamentoStoria >= GlobalVar.avanzamentoStoriaCambioPersonaggio:
...
def dialoga(avanzamentoStoria, personaggio):
    ...
    if avanzamentoStoria < GlobalVar.avanzamentoStoriaCambioPersonaggio:
        imgPersDialogo = pygame.transform.scale(GlobalVar.imgDialogoFraMaggiore, (GlobalVar.gpx * 12, GlobalVar.gpy * 9))
        nomePersonaggio = "Sam"
    else:
        imgPersDialogo = pygame.transform.scale(GlobalVar.imgDialogoSara, (GlobalVar.gpx * 12, GlobalVar.gpy * 9))
        nomePersonaggio = "Sara"

    GlobalVar.schermo.blit(imgPersDialogo, (GlobalVar.gsx // 32 * 19, GlobalVar.gsy // 18 * 4))
    GlobalVar.schermo.blit(personaggio.imgDialogo, (GlobalVar.gsx // 32 * 1, GlobalVar.gsy // 18 * 4))
...
def cambiaProtagonista(nome):
    ...
    GlobalVar.persw = pygame.transform.scale(persw, (GlobalVar.gpx, GlobalVar.gpy))
    perswb = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio4b.png')
    GlobalVar.perswb = pygame.transform.scale(perswb, (GlobalVar.gpx, GlobalVar.gpy))
    persa = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio3.png')
    GlobalVar.persa = pygame.transform.scale(persa, (GlobalVar.gpx, GlobalVar.gpy))
    persab = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio3b.png')
    GlobalVar.persab = pygame.transform.scale(persab, (GlobalVar.gpx, GlobalVar.gpy))
    GlobalVar.perso = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio1.png')
    GlobalVar.perss = pygame.transform.scale(GlobalVar.perso, (GlobalVar.gpx, GlobalVar.gpy))
    persob = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio1b.png')
    GlobalVar.perssb = pygame.transform.scale(persob, (GlobalVar.gpx, GlobalVar.gpy))
    persd = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio2.png')
    GlobalVar.persd = pygame.transform.scale(persd, (GlobalVar.gpx, GlobalVar.gpy))
    persdb = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio2b.png')
    GlobalVar.persdb = pygame.transform.scale(persdb, (GlobalVar.gpx, GlobalVar.gpy))
    perssm = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio1mov.png')
    GlobalVar.perssm = pygame.transform.scale(perssm, (GlobalVar.gpx, GlobalVar.gpy))
    perssmb1 = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio1movb1.png')
    GlobalVar.perssmb1 = pygame.transform.scale(perssmb1, (GlobalVar.gpx, GlobalVar.gpy))
    perssmb2 = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio1movb2.png')
    GlobalVar.perssmb2 = pygame.transform.scale(perssmb2, (GlobalVar.gpx, GlobalVar.gpy))
    persdm = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio2mov.png')
    GlobalVar.persdm = pygame.transform.scale(persdm, (GlobalVar.gpx, GlobalVar.gpy))
    persdmb1 = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio2movb1.png')
    GlobalVar.persdmb1 = pygame.transform.scale(persdmb1, (GlobalVar.gpx, GlobalVar.gpy))
    persdmb2 = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio2movb2.png')
    GlobalVar.persdmb2 = pygame.transform.scale(persdmb2, (GlobalVar.gpx, GlobalVar.gpy))
    persam = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio3mov.png')
    GlobalVar.persam = pygame.transform.scale(persam, (GlobalVar.gpx, GlobalVar.gpy))
    persamb1 = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio3movb1.png')
    GlobalVar.persamb1 = pygame.transform.scale(persamb1, (GlobalVar.gpx, GlobalVar.gpy))
    persamb2 = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio3movb2.png')
    GlobalVar.persamb2 = pygame.transform.scale(persamb2, (GlobalVar.gpx, GlobalVar.gpy))
    perswm = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio4mov.png')
    GlobalVar.perswm = pygame.transform.scale(perswm, (GlobalVar.gpx, GlobalVar.gpy))
    perswmb1 = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio4movb1.png')
    GlobalVar.perswmb1 = pygame.transform.scale(perswmb1, (GlobalVar.gpx, GlobalVar.gpy))
    perswmb2 = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio4movb2.png')
    GlobalVar.perswmb2 = pygame.transform.scale(perswmb2, (GlobalVar.gpx, GlobalVar.gpy))
    perswmbAttacco = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio4movbAttacco.png')
    GlobalVar.perswmbAttacco = pygame.transform.scale(perswmbAttacco, (GlobalVar.gpx, GlobalVar.gpy))
    persambAttacco = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio3movbAttacco.png')
    GlobalVar.persambAttacco = pygame.transform.scale(persambAttacco, (GlobalVar.gpx, GlobalVar.gpy))
    perssmbAttacco = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio1movbAttacco.png')
    GlobalVar.perssmbAttacco = pygame.transform.scale(perssmbAttacco, (GlobalVar.gpx, GlobalVar.gpy))
    persdmbAttacco = pygame.image.load('Immagini/Personaggi/' + nome + '/Personaggio2movbAttacco.png')
    GlobalVar.persdmbAttacco = pygame.transform.scale(persdmbAttacco, (GlobalVar.gpx, GlobalVar.gpy))
    persmbDifesa = pygame.image.load('Immagini/Personaggi/' + nome + '/PersonaggiomovbDifesa.png')
    GlobalVar.persmbDifesa = pygame.transform.scale(persmbDifesa, (GlobalVar.gpx, GlobalVar.gpy))
    persAvvele = pygame.image.load('Immagini/Personaggi/' + nome + '/PersonaggioAvvelenato.png')
    GlobalVar.persAvvele = pygame.transform.scale(persAvvele, (GlobalVar.gpx, GlobalVar.gpy))
