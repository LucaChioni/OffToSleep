...
def attacca(x, y, npers, nrob, rx, ry, pers, pv, pvtot, avvele, attp, difp, enrob, entot, surrisc, velp, effp, stanzaa, stanza, sfondinoa, sfondinob, sfondinoc, portaVert, portaOriz, arma, armatura, scudo, arco, faretra, guanti, collana, robot, armrob, armrobS, attVicino, attLontano, attacco, vitaesca, porte, cofanetti, caseviste, apriocchio, chiamarob, listaNemici, vettoreDenaro, numFrecce, nemicoInquadrato, raffredda, autoRic1, autoRic2, ultimoObbiettivoColco, animaOggetto, listaPersonaggi, startf, avanzamentoStoria):
    ...
    GlobalVar.schermo.blit(persbat, (GlobalVar.gsx // 32 * 0, GlobalVar.gsy // 18 * 17))
    GlobalVar.schermo.blit(GlobalVar.perssb, (GlobalVar.gsx // 32 * 0, GlobalVar.gsy // 18 * 17))
    GlobalVar.schermo.blit(GlobalVar.imgNumFrecce, (GlobalVar.gsx // 32 * 1.2, GlobalVar.gsy // 18 * 17))
    messaggio("x " + str(numFrecce), GlobalVar.grigiochi, GlobalVar.gsx // 32 * 1.8, GlobalVar.gsy // 18 * 17.2, 40)
    if avvele:
        GlobalVar.schermo.blit(GlobalVar.avvelenato, (GlobalVar.gsx // 32 * 3, GlobalVar.gsy // 18 * 17))
    if attp > 0:
        GlobalVar.schermo.blit(GlobalVar.attaccopiu, (GlobalVar.gsx // 32 * 4, GlobalVar.gsy // 18 * 17))
    ...
    while not risposta:
        ...
        if ((xvp / GlobalVar.gpx) + (yvp / GlobalVar.gpy)) % 2 == 0:
            GlobalVar.schermo.blit(sfondinoa, (xvp, yvp))
        if ((xvp / GlobalVar.gpx) + (yvp / GlobalVar.gpy)) % 2 == 1:
            GlobalVar.schermo.blit(sfondinob, (xvp, yvp))
        ...
        if GlobalVar.mouseVisibile:
            ...
            if GlobalVar.gsy // 18 * 17 < yMouse < GlobalVar.gsy and GlobalVar.gsx // 32 * 0 < xMouse < GlobalVar.gsx // 32 * 6:
                ...
                if GlobalVar.mouseBloccato:
                    GlobalVar.configuraCursore(False)
                inquadratoQualcosa = "battaglia"
            elif 0 <= yMouse <= GlobalVar.gsy // 18 * 1.5 and GlobalVar.gsx // 32 * 27.8 < xMouse <= GlobalVar.gsx // 32 * 30.2:
                if GlobalVar.mouseBloccato:
                    GlobalVar.configuraCursore(False)
        ...
        if risposta:
            if ((xp / GlobalVar.gpx) + (yp / GlobalVar.gpy)) % 2 == 0:
                GlobalVar.schermo.blit(sfondinoa, (xp, yp))
            if ((xp / GlobalVar.gpx) + (yp / GlobalVar.gpy)) % 2 == 1:
                GlobalVar.schermo.blit(sfondinob, (xp, yp))
        ...
        if xp == rx and yp == ry:
            ...
            if enrob > 0:
                ...
                while i < len(caseattactotRobo):
                    ...
                    i = i + 3
                campoattaccabile3 = pygame.transform.scale(GlobalVar.campoattaccabileRobo, ((vistaRobo * 2) + GlobalVar.gpx, (vistaRobo * 2) + GlobalVar.gpy))
